...
m.TransportPlan.prototype.releaseShip = function(ship)
{
	...
	{
		API3.warn(" PETRAML: try removing a transporter ship with " + ship.getMetadata(PlayerID, "transporter") +
	...
	for (let entId of this.unloaded)
	{
		...
		else if (!ent.position())  // unloading failed
		{
			...
			if (ship)
			{
				...
				{
					API3.warn("PETRAML transportPlan problem: unit not on ship without position ???");
					this.resetUnit(gameState, ent);
					ent.destroy();
				}
			}
			...
			{
				API3.warn("PETRAML transportPlan problem: unit on ship, but no ship ???");
				this.resetUnit(gameState, ent);
				ent.destroy();
			}
		}
