...
m.getMaxStrength = function(ent, againstClass)
{
	...
	for (let type of attackTypes)
	{
		...
		for (let str in attackStrength)
		{
			...
			case "Pierce":
				...
				break;
			default:
				API3.warn("PETRAML: " + str + " unknown attackStrength in getMaxStrength");
			}
		}
		...
		for (let str in attackTimes)
		{
			...
			case "prepare":
				...
				break;
			default:
				API3.warn("PETRAML: " + str + " unknown attackTimes in getMaxStrength");
			}
		}
	}
	...
	for (let str in armourStrength)
	{
		...
		case "Pierce":
			...
			break;
		default:
			API3.warn("PETRAML: " + str + " unknown armourStrength in getMaxStrength");
		}
	}
...
m.getBestBase = function(gameState, ent, onlyConstructedBase = false, exclude = false)
{
	...
	if (!pos)
	{
		...
		{
			API3.warn("PETRAML error: entity without position, but not garrisoned");
			m.dumpEntity(ent);
