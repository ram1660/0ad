...
m.BuildManager.prototype.incrementBuilderCounters = function(civ, ent, increment)
{
	for (let buildable of ent.buildableEntities(civ))
	{
		if (this.builderCounters.has(buildable))
		{
			...
			if (count < 0)
			{
				API3.warn(" PETRAML error in incrementBuilderCounters for " + buildable + " with count < 0");
				continue;
			}
			...
			API3.warn(" PETRAML error in incrementBuilderCounters for " + buildable + " not yet set");
	}
};
